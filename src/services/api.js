...
const stringifyValidationLocation = (loc) => {
  if (!Array.isArray(loc)) return ''
  return loc
    .map((part) => String(part || '').trim())
    .filter((part) => part.length > 0 && part.toLowerCase() !== 'body')
    .join('.')
}
...
const normalizeErrorDetail = (detail) => {
  ...
  if (detail === null || detail === undefined) return 'Error en la solicitud.'
...
const handleJson = async (response) => {
  ...
  if (!response.ok) {
    const detail = normalizeErrorDetail(data?.detail ?? data?.message)
    const error = new Error(detail || 'Error en la solicitud.')
    error.detail = detail || 'Error en la solicitud.'
...
const deleteEquipmentInspection = async ({ tokenType, accessToken, inspectionId }) => {
  const response = await authFetch(
...
const deleteEquipmentVerification = async ({ tokenType, accessToken, verificationId }) => {
  const response = await authFetch(
...
const deleteEquipmentCalibration = async ({ tokenType, accessToken, calibrationId }) => {
  const response = await authFetch(
...
const fetchTerminalProductTypes = async ({ tokenType, accessToken, terminalId }) => {
  const response = await authFetch(
    `${apiBaseUrl}/api/v1/company-terminals/${terminalId}/product-types`,
    {
      method: 'GET',
      headers: {
        Authorization: `${tokenType || 'bearer'} ${accessToken}`,
...
const fetchTerminalProducts = async ({ tokenType, accessToken, terminalId }) => {
  const response = await authFetch(
    `${apiBaseUrl}/api/v1/company-terminals/${terminalId}/products`,
    {
      method: 'GET',
      headers: {
        Authorization: `${tokenType || 'bearer'} ${accessToken}`,
...
const updateTerminalProductTypes = async ({ tokenType, accessToken, terminalId, productTypes }) => {
  const response = await authFetch(
    `${apiBaseUrl}/api/v1/company-terminals/${terminalId}/product-types`,
    {
      method: 'PUT',
...
const updateTerminalProducts = async ({ tokenType, accessToken, terminalId, products }) => {
  const response = await authFetch(`${apiBaseUrl}/api/v1/company-terminals/${terminalId}/products`, {
    method: 'PUT',
    headers: {
      Authorization: `${tokenType || 'bearer'} ${accessToken}`,
      'Content-Type': 'application/json',
...
export {
  ...
  fetchTerminalProductTypes,
  updateTerminalProductTypes,
