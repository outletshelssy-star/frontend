...
const TerminalsFilters = ({
  ...
  const blockOptions = Array.isArray(blocks) ? blocks : []
  const companyOptions = Array.isArray(companies) ? companies : []
  const normalizedBlockFilter = String(blockFilter ?? 'all')
  const normalizedOwnerFilter = String(ownerFilter ?? 'all')
  const safeBlockFilter =
    normalizedBlockFilter === 'all' ||
    blockOptions.some((block) => String(block?.id || '') === normalizedBlockFilter)
      ? normalizedBlockFilter
      : 'all'
  const safeOwnerFilter =
    normalizedOwnerFilter === 'all' ||
    companyOptions.some((company) => String(company?.id || '') === normalizedOwnerFilter)
