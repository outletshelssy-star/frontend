...
import { useEquipmentCalibrationOpenHandlers } from '../../hooks/useEquipmentCalibrationOpenHandlers'
...
const EquipmentsTable = ({ equipmentTypes, companies, terminals, currentUser }) => {
  ...
  const getCalibrationTooltip = (equipment) => {
    ...
    return (
      <Box sx={{ display: 'grid', gap: 0.25 }}>
        <Typography variant="caption">Ultima: {latest.date.toLocaleDateString()}</Typography>
        <Typography variant="caption">Frecuencia: {frequency}</Typography>
      </Box>
    )
  }
  ...
  const getResolutionDecimals = (resolution) => {
    if (resolution == null) return 2
    if (resolution === 0) return 0
    return Math.min(6, Math.max(0, Math.ceil(-Math.log10(Math.abs(resolution)))))
  }

  const formatSpecValue = (value, resolution) => {
    if (value == null) return '-'
    return Number(value).toFixed(getResolutionDecimals(resolution))
  }
  ...
  return (
    <section className="card dashboard-card">
      ...
      <EquipmentViewDialog
        ...
        getLastVerificationDateLabel={getLastVerificationDateLabel}
      ...
      <InspectionDialog
        ...
        inspectionEditMode={inspectionEditMode}
      ...
      <VerificationDialog
        ...
        verificationEditMode={verificationEditMode}
        canEditVerificationDate={canEditVerificationDate}
