...
const EquipmentsListPanel = ({
  ...
  const availableTerminalOptions = Array.isArray(terminalOptions) ? terminalOptions : []
  const normalizedTerminalFilter = String(terminalFilter ?? 'all')
  const safeTerminalFilter =
    normalizedTerminalFilter === 'all' ||
    availableTerminalOptions.some(
      (terminal) => String(terminal?.id || '') === normalizedTerminalFilter,
    )
      ? normalizedTerminalFilter
      : 'all'
    ...
    <>
      ...
      >
        ...
        <FormControl size="small" sx={{ minWidth: 200 }}>
          <InputLabel id="equipment-terminal-filter">Terminal</InputLabel>
          <Select
            labelId="equipment-terminal-filter"
            value={safeTerminalFilter}
            label="Terminal"
            onChange={(event) => onTerminalFilterChange(event.target.value)}
          >
            <MenuItem value="all">Todos</MenuItem>
            {availableTerminalOptions.map((terminal) => (
                      ...
                      return (
                        ...
                        >
                          {verificationTypesForRow.map((typeItem, index) => {
                            const { status } = getVerificationStatusForType(item, typeItem)
