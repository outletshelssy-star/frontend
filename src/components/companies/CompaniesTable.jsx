...
const COMPANY_FORM_FIELD_ERRORS_INITIAL_STATE = {
  name: '',
  company_type: '',
}
...
const CompaniesTable = () => {
  ...
  const [createFieldErrors, setCreateFieldErrors] = useState(
    COMPANY_FORM_FIELD_ERRORS_INITIAL_STATE,
  )
  ...
  const handleFormDataChange = (changes) => {
    ...
    if (isFormOpen && formMode === 'create') {
      setCreateFieldErrors((prev) => {
        ...
        if (Object.prototype.hasOwnProperty.call(changes, 'company_type')) {
          next.company_type = ''
        }
        return next
      })
  ...
  const openCreate = () => {
    ...
    setCreateFieldErrors(COMPANY_FORM_FIELD_ERRORS_INITIAL_STATE)
    setCreateFocusField('')
    setIsFormOpen(true)
  }
  ...
  const openEdit = (company) => {
    ...
    setEditFieldErrors(COMPANY_FORM_FIELD_ERRORS_INITIAL_STATE)
    setEditFocusField('')
    setIsFormOpen(true)
  }
  ...
  const handleCreate = async () => {
    ...
    if (!String(formData.company_type || '').trim()) {
      nextFieldErrors.company_type = 'Selecciona un tipo.'
    }
    setCreateFieldErrors(nextFieldErrors)
    const firstFieldWithError = ['name', 'company_type'].find((field) => nextFieldErrors[field])
  ...
  return (
    <section className="card dashboard-card">
      ...
      <CompanyFormDialog
        open={isFormOpen}
        title={isCreateMode ? 'Nueva empresa' : 'Editar empresa'}
        formData={formData}
      ...
      <CompanyProgressDialog
        ...
        title={isCreateLoading ? 'Guardando empresa...' : 'Actualizando empresa...'}
      />
