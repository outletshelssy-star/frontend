...
const CREATE_SELECT_ERRORS_INITIAL_STATE = {
  role: '',
  measure_to_add: '',
  units: {},
}

const CREATE_FIELD_ERRORS_INITIAL_STATE = {
  name: '',
  max_error_values: {},
}

const EDIT_SELECT_ERRORS_INITIAL_STATE = {
  role: '',
  measure_to_add: '',
  units: {},
}

const EDIT_FIELD_ERRORS_INITIAL_STATE = {
  name: '',
  max_error_values: {},
}
...
const EquipmentTypesTable = ({ currentUser }) => {
  ...
  const [createSelectErrors, setCreateSelectErrors] = useState(CREATE_SELECT_ERRORS_INITIAL_STATE)
  const [createFieldErrors, setCreateFieldErrors] = useState(CREATE_FIELD_ERRORS_INITIAL_STATE)
  const [editSelectErrors, setEditSelectErrors] = useState(EDIT_SELECT_ERRORS_INITIAL_STATE)
  const [editFieldErrors, setEditFieldErrors] = useState(EDIT_FIELD_ERRORS_INITIAL_STATE)
  ...
  const openCreate = () => {
    ...
    setMaxErrors({})
    setCreateSelectErrors({ ...CREATE_SELECT_ERRORS_INITIAL_STATE, units: {} })
    setCreateFieldErrors({ ...CREATE_FIELD_ERRORS_INITIAL_STATE, max_error_values: {} })
  ...
  const openEdit = (item) => {
    setFormMode('edit')
    setEditSelectErrors({ ...EDIT_SELECT_ERRORS_INITIAL_STATE, units: {} })
    setEditFieldErrors({ ...EDIT_FIELD_ERRORS_INITIAL_STATE, max_error_values: {} })
  ...
  const handleMeasuresChange = (value) => {
    ...
    if (isEditOpen) {
      setEditSelectErrors((prev) => {
        const nextUnitErrors = {}
        nextMeasures.forEach((measure) => {
          if (prev.units?.[measure]) {
            nextUnitErrors[measure] = prev.units[measure]
          }
        })
        return {
          ...prev,
          measure_to_add: nextMeasures.length > 0 ? '' : prev.measure_to_add,
          units: nextUnitErrors,
        }
      })
      ...
      setEditFieldErrors((prev) => {
        ...
        nextMeasures.forEach((measure) => {
          if (prev.max_error_values?.[measure]) {
            nextValueErrors[measure] = prev.max_error_values[measure]
          }
        })
        return {
          ...prev,
          max_error_values: nextValueErrors,
        }
      })
    } else {
      setCreateSelectErrors((prev) => {
        const nextUnitErrors = {}
        nextMeasures.forEach((measure) => {
          if (prev.units?.[measure]) {
            nextUnitErrors[measure] = prev.units[measure]
  ...
  const validateForm = ({
    ...
    skipMeasureSelection = false,
    skipUnitValidation = false,
    skipMaxErrorValueValidation = false,
  } = {}) => {
    if (!skipNameValidation && !formData.name.trim()) {
      return 'El nombre es obligatorio.'
    }

    if (!skipMeasureSelection && formData.has_measures && !measures.length) {
    ...
    if (formData.has_measures) {
      if (!skipUnitValidation) {
        const missingUnit = measures.find((measure) => !String(maxErrors[measure]?.unit || '').trim())
        if (missingUnit) {
          return 'Selecciona la unidad para cada medida.'
        }
      }
      if (!skipMaxErrorValueValidation) {
        const missingValue = measures.find(
          (measure) =>
        ...
        if (missingValue) {
          return 'Completa el valor de error maximo para cada medida.'
        }
      }
    }
  ...
  const handleCreate = async () => {
    ...
    if (!String(formData.role || '').trim()) {
      nextSelectErrors.role = 'Selecciona el rol.'
    }
    if (formData.has_measures && !measures.length) {
      nextSelectErrors.measure_to_add = 'Selecciona al menos una medida.'
    }
    if (formData.has_measures && measures.length > 0) {
      measures.forEach((measure) => {
        if (!String(maxErrors[measure]?.unit || '').trim()) {
          nextSelectErrors.units[measure] = 'Selecciona la unidad.'
        }
        if (
          maxErrors[measure]?.max_error_value === '' ||
          maxErrors[measure]?.max_error_value === null ||
          ...
          nextFieldErrors.max_error_values[measure] = 'Completa el valor.'
        }
      })
    }
    setCreateFieldErrors(nextFieldErrors)
    setCreateSelectErrors(nextSelectErrors)
    if (
      nextFieldErrors.name ||
      Object.keys(nextFieldErrors.max_error_values).length > 0 ||
      nextSelectErrors.role ||
      nextSelectErrors.measure_to_add ||
      Object.keys(nextSelectErrors.units).length > 0
    ) {
      return
    }
    ...
    const error = validateForm({
      ...
      skipMeasureSelection: true,
      skipUnitValidation: true,
      skipMaxErrorValueValidation: true,
    })
  ...
  const handleUpdate = async () => {
    ...
    setEditSelectErrors(nextSelectErrors)
    if (
      nextFieldErrors.name ||
      Object.keys(nextFieldErrors.max_error_values).length > 0 ||
      nextSelectErrors.role ||
        ...
        >
          ...
          >
            ...
            >
              ...
              <FormControl error={Boolean(createSelectErrors.role)}>
                ...
                <Select
                  ...
                  onChange={(event) => {
                    ...
                    setCreateSelectErrors((prev) => ({ ...prev, role: '' }))
                  }}
                >
                ...
                {createSelectErrors.role ? (
                  <FormHelperText>{createSelectErrors.role}</FormHelperText>
                ) : null}
              </FormControl>
            </Box>
          </Box>
          ...
          >
            ...
            >
              ...
              >
                <FormControl error={Boolean(createSelectErrors.measure_to_add)}>
                  ...
                  <Select
                    ...
                    value={measureToAdd}
                    onChange={(event) => {
                      setMeasureToAdd(event.target.value)
                      setCreateSelectErrors((prev) => ({ ...prev, measure_to_add: '' }))
                    }}
                  ...
                  <FormHelperText>
                    {createSelectErrors.measure_to_add || 'Selecciona una medida y agregala a la lista.'}
                  </FormHelperText>
                </FormControl>
                      ...
                      <TextField
                        ...
                        onChange={(event) => {
                          ...
                          setCreateFieldErrors((prev) => ({
                            ...prev,
                            max_error_values: {
                              ...prev.max_error_values,
                              [measure]: '',
                            },
                          }))
                        }}
                        error={Boolean(createFieldErrors.max_error_values?.[measure])}
                        helperText={createFieldErrors.max_error_values?.[measure] || ''}
                      />

                      <FormControl
                        sx={{ minWidth: 130 }}
                        error={Boolean(createSelectErrors.units?.[measure])}
                      >
                        ...
                        {createSelectErrors.units?.[measure] ? (
                          <FormHelperText>{createSelectErrors.units[measure]}</FormHelperText>
                        ) : null}
                      </FormControl>
        ...
        >
          ...
          >
            ...
            >
              ...
              <FormControl error={Boolean(editSelectErrors.role)}>
                ...
                <Select
                  ...
                  onChange={(event) => {
                    setFormData((prev) => ({ ...prev, role: event.target.value }))
                    setEditSelectErrors((prev) => ({ ...prev, role: '' }))
                  }}
                >
                  {ROLE_OPTIONS.map((role) => (
                    <MenuItem key={role.value} value={role.value}>
                ...
                {editSelectErrors.role ? <FormHelperText>{editSelectErrors.role}</FormHelperText> : null}
              </FormControl>
          ...
          >
            ...
            >
              <FormControl error={Boolean(editSelectErrors.measure_to_add)}>
                ...
                {editSelectErrors.measure_to_add ? (
                  <FormHelperText>{editSelectErrors.measure_to_add}</FormHelperText>
                ) : null}
              </FormControl>
                  ...
                  {measures.map((measure) => (
                    ...
                    >
                      ...
                      <TextField
                        ...
                        error={Boolean(editFieldErrors.max_error_values?.[measure])}
                        helperText={editFieldErrors.max_error_values?.[measure] || ''}
                      />

                      <FormControl
                        sx={{ minWidth: 130 }}
                        error={Boolean(editSelectErrors.units?.[measure])}
                      >
                        ...
                        {editSelectErrors.units?.[measure] ? (
                          <FormHelperText>{editSelectErrors.units[measure]}</FormHelperText>
                        ) : null}
                      </FormControl>
                    </Box>
                  ))}
